...
def preprocess(adj, features, labels, preprocess_adj=False, preprocess_feature=False, onehot_feature= False,sparse=False, device='cpu'):
    """Convert adj, features, labels from array（数组） or sparse matrix to
    ...
    device : str
        'cpu' or 'cuda'
    """
    # adj, features, labels, preprocess_adj = False, preprocess_feature = False,
    # onehot_feature = False, sparse = False, device = 'cpu'

    if preprocess_adj:#False
        adj_norm = normalize_adj(adj)

    if preprocess_feature:#False
...
def preprocessmlp(features, labels, preprocess_adj=False, preprocess_feature=False, onehot_feature= False,sparse=False):
    """Convert adj, features, labels from array（数组） or sparse matrix to
    torch Tensor, and normalize the input data.
...
def normalize_adj_tensor1(adj, sparse=False):
    """Normalize adjacency tensor matrix.
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")#"cuda:0" if torch.cuda.is_available() else "cpu"
    if sparse:
        ...
        # ✅ 如果 adj 是 SciPy 稀疏矩阵，转换为稠密 Tensor
...
def normalize_adj_tensor(adj, sparse=False):
    """Normalize adjacency tensor matrix.
    """#device = torch.device("cuda" if adj.is_cuda else "cpu")第一行不可用
    device = torch.device("cuda" if adj.is_cuda else "cpu")#"cuda:0" if torch.cuda.is_available() else "cpu"
    if sparse:
        # TODO if this is too slow, uncomment the following code,
        # but you need to install torch_scatter
        # return normalize_sparse_tensor(adj)
...
def get_train_val_test_gcn_different_distribution(labels, seed=None):
    ...
    for i in val_test_classes:
        labels_i = idx[labels == i]
        labels_i = np.random.permutation(labels_i)
        idx_val = np.hstack((idx_val, labels_i[:len(labels_i) // 2])).astype(int)
        idx_test = np.hstack((idx_test, labels_i[len(labels_i) // 2:])).astype(int)
